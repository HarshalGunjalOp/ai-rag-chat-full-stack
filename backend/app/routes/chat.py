...
from app.models.schemas import (
    MessageRequest, MessageResponse, ConversationCreate, 
    ConversationResponse, RAGQueryRequest, RAGResponse, 
    ChatResponse, FileUploadResponse, MessageType
)
...
async def get_conversation_messages(conversation_id: int, limit: int = 50):
    ...
    try:
        ...
        cached_messages = await cache_service.get_recent_messages(str(conversation_id), limit)
        if cached_messages:
            ...
            for msg_data in cached_messages:
                try:
                    ...
                    if 'text' not in msg_data['content']:
                        if isinstance(msg_data['content'], dict) and len(msg_data['content']) > 0:
                            msg_data['content']['text'] = str(list(msg_data['content'].values())[0])
                        else:
                            msg_data['content']['text'] = ""
        ...
        async with await db_manager.get_connection() as conn:
            ...
            for row in results:
                try:
                    ...
                    print(f"Error processing DB message {row.get('id', 'unknown')}: {e}")
                    continue
...
async def generate_ai_response(query: str, conversation_id: int, user_id: str):
    ...
    try:
        ...
        async for chunk in rag_service.query(query, user_id=user_id):  # Added user_id parameter
...
async def save_message(conversation_id: int, user_id: str, content: dict, message_type: str) -> dict:
    ...
    async with await db_manager.get_connection() as conn:
        result = await conn.fetchrow(
            ...
            conversation_id, user_id, json.dumps(content), message_type
        )
        return dict(result)
...
async def stream_rag_response(query: str):
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": f"This is the context provided by the user. If you find the answer to the query in the context, then answer based on that, else use your own knowlege to answer the question: {context}"},
    ...
    try:
        ...
        if not conversation_id:
            async with await db_manager.get_connection() as conn:
                conv_result = await conn.fetchrow(
                    "INSERT INTO conversations (user_id, title) VALUES ($1, $2) RETURNING id",
                    message_req.user_id, "New Conversation"
                )
        ...
        async with await db_manager.get_connection() as conn:
            user_msg_result = await conn.fetchrow("""
                ...
                VALUES ($1, $2, $3, $4) 
                RETURNING id, conversation_id, user_id, content, message_type, created_at
            """, conversation_id, message_req.user_id, 
                user_content_json, MessageType.USER.value
            )
        ...
        rag_response = None
        if await rag_service.has_documents(message_req.user_id):  # Added user_id parameter
            rag_start = time.time()
            ...
            if cached_rag:
                ...
                assistant_content_dict = {
                    ...
                    "search_method": rag_result.get("search_method", "hybrid_multimodal"),
                    "response_type": "rag"
                }
...
async def upload_document(
    ...
    try:
        ...
        allowed_extensions = {'.pdf', '.txt', '.md', '.jpg', '.jpeg', '.png', '.bmp', '.gif'}
...
async def query_rag(rag_query: RAGQueryRequest):
    ...
    try:
        ...
        if not await rag_service.has_documents(rag_query.user_id):  # Added user_id parameter
...
async def get_document_status(user_id: str):
    ...
    try:
        ...
        return {
            ...
            "supported_formats": [".pdf", ".txt", ".md", ".jpg", ".jpeg", ".png", ".bmp", ".gif"]
        }
    except Exception as e:
