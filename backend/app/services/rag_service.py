...
class RAGService:
    ...
    async def query(self, query: str, user_id: str) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            if await self.has_documents(user_id):
                ...
                relevant_docs = await self.hybrid_search(query, user_id, self.hybrid_config["default_topk"])
                
                if relevant_docs and self.get_max_relevance_score(relevant_docs) > self.hybrid_config["min_relevance_threshold"]:
                    async for chunk in self.generate_context_answer_stream(query, relevant_docs, user_id):
                        if chunk["type"] == "complete":
                            # Cache the response
                            full_response = ""  # You'd need to collect this during streaming
                            cache_data = {
    ...
    async def generate_general_knowledge_answer_stream(self, query: str):
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": "You are a helpful assistant that answers questions based on provided context if available, otherwise provide general knowledge responses."},
    ...
    async def generate_context_answer_stream(self, query: str, relevant_docs: List[Document], user_id: str):
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": "You are a helpful assistant that answers questions based on the user's uploaded documents. Be precise and cite information appropriately."},
                {"role": "user", "content": f"Context from user's documents:\n{context}\n\nQuestion: {query}\n\nAnswer based on the context above."}
            ]
    ...
    async def hybrid_search(self, query: str, user_id: str, topk: int) -> List[Document]:
    ...
    async def semantic_search(self, query: str, user_id: str, topk: int) -> List[Document]:
        ...
        scores, indices = self.user_faiss_indexes[user_id].search(query_vector, min(topk, len(user_docs)))
    ...
    async def keyword_search(self, query: str, user_id: str, topk: int) -> List[Document]:
    ...
    def combine_results(self, semantic_results: List[Document], keyword_results: List[Document]) -> List[Document]:
        ...
        combined_results = []
        max_keyword_score = max([scores['keyword'] for scores in score_map.values()]) or 1.0
        
        for chunk_id, scores in score_map.items():
            keyword_score = scores['keyword'] / max_keyword_score if max_keyword_score > 0 else 0
            combined_score = alpha * scores['semantic'] + (1 - alpha) * keyword_score
            doc = scores['doc']
            doc.metadata['combined_score'] = combined_score
            combined_results.append(doc)
        
        combined_results.sort(key=lambda x: x.metadata.get('combined_score', 0), reverse=True)
...
Return the expanded query (original + related terms):"""
    ...
    async def process_multimodal_document(self, filename: str, content: bytes, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            processing_context = {
                "user_id": user_id,
                "filename": filename,
                "original_filename": filename.split(f"{user_id}_")[1] if f"{user_id}_" in filename else filename,
            ...
            if file_ext == '.pdf':
                return await self.process_pdf_advanced(filename, content, user_id, processing_context)
            elif file_ext == '.txt':
                return await self.process_text_file(filename, content, user_id, processing_context)
            elif file_ext == '.md':
                return await self.process_markdown_file(filename, content, user_id, processing_context)
            elif file_ext in ['.jpg', '.jpeg', '.png', '.bmp', '.gif']:
                return await self.process_image_file(filename, content, user_id, processing_context)
            else:
    ...
    async def process_pdf_advanced(self, filename: str, content: bytes, user_id: str, context: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return await self.process_text_content(filename, full_text, 'pdf', user_id, context)
            
        except Exception as e:
            print(f"PDF processing error for user {user_id}: {e}")
            raise

    async def process_text_file(self, filename: str, content: bytes, user_id: str, context: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return await self.process_text_content(filename, text, 'text', user_id, context)
        except Exception as e:
            print(f"Text file processing error for user {user_id}: {e}")
            raise

    async def process_markdown_file(self, filename: str, content: bytes, user_id: str, context: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return await self.process_text_content(filename, text, 'markdown', user_id, context)
            
        except Exception as e:
            print(f"Markdown processing error for user {user_id}: {e}")
            raise

    async def process_image_file(self, filename: str, content: bytes, user_id: str, context: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = [
                HumanMessage(content=[
                    {"type": "text", "text": "Describe this image in detail, including any text, objects, people, and context you can see."},
    ...
    async def process_text_content(self, filename: str, text: str, doc_type: str, user_id: str, context: Dict) -> Dict[str, Any]:
    ...
    async def index_user_documents(self, docs: List[Document], user_id: str):
        ...
        print(f"Indexed {len(docs)} chunks for user {user_id}. Total user documents: {len(self.user_documents[user_id])}")
