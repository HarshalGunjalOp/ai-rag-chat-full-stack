...
export default function UploadModal({open, onClose}: {open: boolean; onClose: () => void}) {
  ...
  useEffect(() => {
    ...
    api.uploadFiles(userId, list)
      .then(res => {
        setFiles(res.map((r: any) => ({name: r.filename, status: r.status === 'processing' ? 'ok' : 'error'})));
        refreshDocStatus();
      })
  ...
  return (
    <Dialog open={open} onClose={onClose} className="text-white bg-slate-800 fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <Dialog.Panel className="bg-dark-secondary p-6 rounded-lg max-w-md w-full">
        <Dialog.Title className="text-lg font-semibold text-white mb-4">Uploading Documents</Dialog.Title>
